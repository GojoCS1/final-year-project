...
<html>
<head>
    <title>Nurse Dashboard - Adigrat Hospital</title>
    <style>
        .form-container {
...
<div class="main-content">
    <div class="card" id="display-area">
        <h1>Welcome, Nurse <?php echo htmlspecialchars($current_user['full_name']); ?></h1>
...
filterStaffByArea('lab', '', 'lab_tech_select');
    ...
    if (!results || results.length === 0) {
        container.innerHTML = `<p style="color:#999; font-style:italic;">No lab results found for this patient in the last 12 hours.</p>`;
        return;
    }
    
    let html = `<table style="width:100%; border-collapse:collapse; border:2px solid darkcyan;">
        <thead>
            <tr style="background:white; color:darkcyan;">
    ...
    area.innerHTML = `<h1>Re-Assign Patient</h1>${idHeader} ${demoHTML}
        <div style="background:#f0f7f7; padding:15px; border-radius:8px; border:1px solid darkcyan;">
            ...
            <div class="form-group">
                ...
                <select name="re_assign_to_staff_" id="re_assign_to_staff_">
                    <option value="">Select location first...</option>
                </select>
            </div>
            <button class="action-btn" onclick="saveToDB('Reassignment', {id: '${patientId}'})">Confirm Re-assignment</button>
        </div>`;
}
    ...
    if (h.action_type === 'Order Sheet') {
        ...
        Object.entries(h).forEach(([k, v]) => {
            if (!['id','patient_id','mrn','staff_id','created_at','action_type','row_index','days_checked','ward','bed_number','status','record_date','is_read'].includes(k) && v) {
                ...
                // Handle Image display in history
                if (k === 'result_image') {
...
// 3. ተረኛ ሰራተኛ (ዶክተር/ነርስ/ላብ/ፋርማሲ) በሽፍት ብቻ መፈለጊያ
...
// Check and display notification count
...
// Poll every 10 seconds + run on page load
